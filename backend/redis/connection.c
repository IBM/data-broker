...
//  dbBE_Redis_sr_buffer_t *sendb = dbBE_Transport_sr_buffer_allocate( sr_buffer_size );
  recvb = dbBE_Transport_dbuffer_allocate( sr_buffer_size );
  ...
  cmd = dbBE_Transport_sge_buffer_create();
  if( cmd == NULL )
  {
    rc = ENOMEM;
    goto error;
...
error:
  if( recvb != NULL )
    dbBE_Transport_dbuffer_free( recvb );
  if( slots != NULL )
    dbBE_Redis_slot_bitmap_destroy( slots );
  if( cmd != NULL )
    dbBE_Transport_sge_buffer_destroy( cmd );
  if( conn )
    free( conn );
...
int dbBE_Redis_connection_assign_recvbuf( dbBE_Redis_connection_t *conn,
                                          dbBE_Transport_dbuffer_t *recvb )
{
  if( conn == NULL )
    return -EINVAL;
...
ssize_t dbBE_Redis_connection_recv_base( dbBE_Redis_connection_t *conn, dbBE_Redis_sr_buffer_t *buf )
{
  ...
  do
  {
    if( dbBE_Transport_sr_buffer_remaining( buf ) - 4 <= 0 )
    {
    ...
    // receive 7/8th at max
    ssize_t rsize = dbBE_Transport_sr_buffer_remaining( buf ) - (dbBE_Transport_sr_buffer_remaining( buf ) >> 4);
    ...
    LOG( DBG_TRACE, stderr, "recv( %d, %p, %ld, 0) = %ld\n", conn->_socket, dbBE_Transport_sr_buffer_get_available_position( buf ), rsize, rc );
                                    ...
                                    dbBE_Redis_sr_buffer_t *buf )
{
  ...
  int empty = dbBE_Transport_sr_buffer_empty( buf );
  if( ! empty )
    return -ENOTEMPTY;
  ...
  ssize_t rc = dbBE_Redis_connection_recv_base( conn, buf );

  LOG( DBG_TRACE, stdout, "RECV: conn=%d:%s", conn->_socket, dbBE_Transport_sr_buffer_get_start( buf ) );
  ...
  if( dbBE_Transport_sr_buffer_available( buf ) > 1000 )
  {
    ...
    LOG( DBG_ALL, stderr, "RECV last bytes: %x %x %x %x (total:%"PRId64"\n",
         logptr[0], logptr[1], logptr[2], logptr[3], dbBE_Transport_sr_buffer_available( buf ) );
  }
#endif
  return rc;
}
                                         ...
                                         dbBE_Redis_sr_buffer_t *buf )
{
  ...
  LOG( DBG_VERBOSE, stdout, "recv_more: conn=%d; new=%zd; avail/rem=%zd/%zd\n",
       conn->_socket, rc, dbBE_Transport_sr_buffer_available( buf ), dbBE_Transport_sr_buffer_remaining( buf ) );
  LOG( DBG_TRACE, stdout, "recv_more: conn=%d:%s", conn->_socket, dbBE_Transport_sr_buffer_get_start( buf ) );
                                        ...
                                        dbBE_Transport_sge_buffer_t *sb )
{
  ...
  msg.msg_iov = sb->_cmd;
  msg.msg_iovlen = sb->_index;

  return recvmsg( conn->_socket, &msg, 0 );
}
